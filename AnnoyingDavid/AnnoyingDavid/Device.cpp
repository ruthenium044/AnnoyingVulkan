...
namespace svk {
    ...
    void Device::createInstance() {
        ...
        //extensions
        const auto extensionNames = getRequiredExtensions();
    ...
    bool Device::isDeviceSuitable(VkPhysicalDevice device) {
        ...
        // DISCRETE ONLY
        if (props.deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) { return false; }
    ...
    std::vector<const char*> Device::getRequiredExtensions() const {
        ...
        std::vector<const char*> extensionNames{static_cast<size_t>(extensionCount)};
        if (!SDL_Vulkan_GetInstanceExtensions(window->GetWindow(), &extensionCount, extensionNames.data())) {
            printf(SDL_GetError());
        }

        if (enableValidationLayers) { extensionNames.push_back(VK_EXT_DEBUG_UTILS_EXTENSION_NAME); }
